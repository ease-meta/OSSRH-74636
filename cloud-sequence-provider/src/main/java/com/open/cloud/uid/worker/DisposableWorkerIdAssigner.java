...
import com.open.cloud.uid.context.UidGeneratorContext;
import com.open.cloud.uid.entity.WorkerNodePo;
import com.open.cloud.uid.exception.UidGenerateException;
import com.open.cloud.uid.utils.NetUtils;
...
public class DisposableWorkerIdAssigner {
    ...
    com.open.cloud.uid.repository.WorkerNodePoRepository workerNodePoRepository;
    ...
    public WorkerNodePo selectByBizTagAndTypeTodoForUpdateOrInsert(String type, String bizTag) {
        ...
        if (dbTags == null || dbTags.isEmpty()) {
            ...
            workerNodePo.setHostName(com.open.cloud.uid.utils.NetUtils.getLocalAddress());
            ...
            WorkerNodePo needUpdate = dbTags.stream().filter(v -> bizTag.equals(v.getBizTag()) && StringUtils.equals(com.open.cloud.uid.utils.NetUtils.getLocalAddress(), v.getHostName())).sorted(Comparator.comparing(WorkerNodePo::getMaxSeq).reversed()).findFirst().orElseGet(() -> {
                log.info("存在相同的序列类型{}，在{}节点生成新的实例", bizTag, com.open.cloud.uid.utils.NetUtils.getLocalAddress());
                WorkerNodePo workerNodePo1 = new WorkerNodePo();
                workerNodePo1.setHostName(com.open.cloud.uid.utils.NetUtils.getLocalAddress());
    ...
    public List<WorkerNodePo> findByTypeIfNullInsert(String type) {
        ...
        if (dbTags == null || dbTags.isEmpty()) {
            ...
            if (UidGeneratorContext.UID_DEF_DEF.equalsIgnoreCase(type)) {
                WorkerNodePo needUpdate = dbTags.stream().filter(v -> type.equals(v.getBizTag()) && StringUtils.equals(com.open.cloud.uid.utils.NetUtils.getLocalAddress(), v.getHostName())).sorted(Comparator.comparing(WorkerNodePo::getMaxSeq).reversed()).findFirst().orElseGet(() -> {
                    log.info("存在相同的序列类型{}，在{}节点生成新的实例", type, com.open.cloud.uid.utils.NetUtils.getLocalAddress());
                    WorkerNodePo workerNodePo1 = new WorkerNodePo();
                    workerNodePo1.setHostName(com.open.cloud.uid.utils.NetUtils.getLocalAddress());
                    workerNodePo1.setPort(System.currentTimeMillis() + "-" + RandomUtils.nextInt(100000));
                    workerNodePo1.setType(type);
    ...
    public void setWorkerNodePoRepository(com.open.cloud.uid.repository.WorkerNodePoRepository workerNodePoRepository) {
