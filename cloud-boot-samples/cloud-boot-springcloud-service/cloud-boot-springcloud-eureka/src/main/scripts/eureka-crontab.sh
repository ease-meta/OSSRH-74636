...
BASE_DIR=`pwd`
# 创建定时任务文件
CRON_COUNT=`crontab -l| grep -c "eureka-crontab.sh"`
if [ $CRON_COUNT -eq 0 ];then
        crontab -l >> $BASE_DIR/bin/cron.everyday
        echo "0 0 */1 * * $BASE_DIR/bin/eureka-crontab.sh" >> $BASE_DIR/bin/cron.everyday
...
{
    cp $BASE_DIR/log/stdout.log $BASE_DIR/log/eureka-${yesterdayDate}.log
	echo > $BASE_DIR/log/stdout.log
	rm -rf $BASE_DIR/log/eureka-${oldDate}*
}
exe_dependencies