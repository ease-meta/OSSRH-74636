...
public class LimiterService {
	...
	public LimiterDefinitionDTO resolverLimiterToDTO(final LimiterDefinition limiterDefinition) {
		try {
			...
			final List<ParamListInformation> reList = Optional.ofNullable(resolverList).orElse(JSON.parseArray(FileUtils.readFileToString(new File(this.resolverPath), "UTF-8"), ParamListInformation.class));
			...
			if (limiterDefinition.getPredicate() != null) {
				...
				final List<ParamListInformation> paList = Optional.ofNullable(predicateList).orElse(JSON.parseArray(FileUtils.readFileToString(new File(this.predicatePath), "UTF-8"), (Class) ParamListInformation.class));
