...
public class TestString {
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        //String所占空间：char（2字节）个数*2+（8 字节）+ 引用 (4 字节 )  + char 数组（16 字节）+ 1个 int（4字节）+ 1个long（8字节）
        ...
        System.out.println("\"搞java\".getBytes(\"UTF-8\").length:" + (str.getBytes(StandardCharsets.UTF_8).length));
/**
 "搞java".length():5
