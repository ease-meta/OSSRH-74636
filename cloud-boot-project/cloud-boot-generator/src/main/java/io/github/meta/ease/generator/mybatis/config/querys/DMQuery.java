...
public class DMQuery extends AbstractDbQuery {
    ...
    public String tableFieldsSql() {
        return "SELECT T2.COLUMN_NAME,T1.COMMENTS,"
                ...
                + "AND T5.CONSTRAINT_TYPE = 'P')T3 " + "WHERE T1.TABLE_NAME = T2.TABLE_NAME AND "
                + "T1.COLUMN_NAME=T2.COLUMN_NAME AND " + "T1.TABLE_NAME = T3.TABLE_NAME(+) AND "
                + "T1.COLUMN_NAME=T3.COLUMN_NAME(+)   AND " + "T1.TABLE_NAME = '%s' "
