...
public class KeyGenerateFactory {
    ...
    private Map<String, com.moc.cloud.workflow.framework.idgenerator.TimeSequenceGenerator> keys = new ConcurrentHashMap<>();
    ...
    public synchronized String getKey(String name) {
        com.moc.cloud.workflow.framework.idgenerator.TimeSequenceGenerator key = keys.get(name);
        if (key == null) {
            key = new com.moc.cloud.workflow.framework.idgenerator.TimeSequenceGenerator(name);
            keys.put(name, key);
        }
        return key.nextId();
    }
