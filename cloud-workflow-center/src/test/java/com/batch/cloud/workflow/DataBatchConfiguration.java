...
public class DataBatchConfiguration {
    ...
    @Bean
    public CheckBatchStatus checkBatchStatus() {
        return new CheckBatchStatus();
    }

    @Bean
    protected Step checkBatchStatusStep() {
        return stepBuilderFactory
                .get("checkBatchStatus")
                .tasklet(checkBatchStatus())
                .build();
    }

    @Bean
    public Job checkBatchStatusJob() {
        return jobBuilderFactory.get("checkBatchStatus").
                incrementer(new RunIdIncrementer()).
                start(checkBatchStatusStep()).    //start是JOB执行的第一个step
//                next(xxxStep()).
//                next(xxxStep()).
//                ...
