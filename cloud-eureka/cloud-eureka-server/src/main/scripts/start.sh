...
if [ -n "$IS_LISTENED" ]; then
    echo "ERROR: The [$SERVER_PORT] already started!"
    exit 1
fi
...
echo -e "Starting ...\c"
$JAVA_HOME/bin/java $JVM_OPTS -classpath $CONF_DIR:$LIB_JARS com.dcits.EurekaServerApplication >> $STDOUT_FILE 2>&1 &
./bin/eureka-crontab.sh
