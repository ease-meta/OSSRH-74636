...
import com.open.cloud.config.CatChunkListener;
import com.open.cloud.listener.JobListener;
import com.open.cloud.model.Access;
...
public class DataBatchConfiguration {
    ...
    public Job dataHandleJob() {
        return jobBuilderFactory.get("dataHandleJob").incrementer(new RunIdIncrementer())
    ...
    public Step handleDataStep() {
        return stepBuilderFactory.get("getData").<Access, Access> chunk(100). // <输入,输出> 。chunk通俗的讲类似于SQL的commit; 这里表示处理(processor)100条后写入(writer)一次。
            ...
            listener(catChunkListener).build();
    }
